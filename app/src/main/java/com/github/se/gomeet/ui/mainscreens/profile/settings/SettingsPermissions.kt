      ...
      bottomBar = {
        ...
        // Composable Content
        {
              ...
              val settingsLauncher =
                  rememberLauncherForActivityResult(
                      ActivityResultContracts.StartActivityForResult()) { result ->
                        // after returning from the settings, update the permissions status to catch
                        // an changes
                        updatePermissions(context, permissionsState)
                      }
              ...
              fun permissionCompagnon(perm: String): () -> Unit {
                return {
                  // if the permission is already granted, launch a dialogue explaining the
              ...
              if (showPermissionDialog) {
                SettingsDialogue(
                    ...
                    text =
                        "The app requires some permissions from you device. To enable the full functionality of the app, please go to settings and enable the permission manually",
                )
              }
              if (showConfirmation) {
                SettingsDialogue(
                    ...
                    title = "Are You Sure ?",
                    text =
                        "Are you sure you want to disable this permission ? Doing so might prevent some functionalities of the app from working properly",
                )
              }
            }
      }
...
/**
 * Function to get the state of a permission as a string Will also check coarse location when passed
 * FINE_LOCATION
 *
...
fun getPermissionState(
  ...
  else if (permissionString == ACCESS_COARSE_LOCATION &&
      permissionsState.value.get(ACCESS_COARSE_LOCATION) == 0) {
...
/**
 * Composable representing a dialogue screen to be opened when the user needs to access the
 * permission settings Show a title, a text, a button toward setting and a cancel button
 *
