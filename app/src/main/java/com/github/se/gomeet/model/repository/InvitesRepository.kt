...
class InvitesRepository(private val db : FirebaseFirestore) {
    ...
    private fun updateEventInvites(eventID: String, userID: String, status: InviteStatus, success: (Boolean) -> Unit) {
        ...
        db.runTransaction { transaction ->
            ...
            val currentUsers = if (snapshot.exists()) {
                snapshot.toObject(EventInviteUsers::class.java)?.usersInvited?.toMutableList() ?: mutableListOf()
            } else {
                mutableListOf()
            }
    ...
    private fun updateUserInvites(userID: String, eventID: String, status: InviteStatus, success: (Boolean) -> Unit) {
        ...
        db.runTransaction { transaction ->
            ...
            val currentEvents = if (snapshot.exists()) {
                snapshot.toObject(UserInvitedToEvents::class.java)?.invitedToEvents?.toMutableList() ?: mutableListOf()
            } else {
                mutableListOf()
            }
    ...
    fun removeInvite(eventID: String, toUserID: String) {
        ...
        db.runTransaction { transaction ->
            ...
            val updatedEventUsers = eventSnapshot.data!!.toEventInviteUsers(eventID).usersInvited.filterNot { it.first == toUserID }.toMutableList()
            ...
            val updatedUserEvents = userSnapshot.data!!.toUsersInvitedToEvents(toUserID).invitedToEvents.filterNot { it.first == eventID }.toMutableList()
            transaction.set(userInviteRef, UserInvitedToEvents(toUserID, updatedUserEvents))
        }.addOnSuccessListener {
            Log.d(TAG, "Transaction successful: both invites removed")
        }.addOnFailureListener { e ->
    ...
    private fun Map<String, Any>.toEventInviteUsers(id: String? = null): EventInviteUsers {
        return EventInviteUsers(
            ...
            usersInvited = this["usersInvited"] as? MutableList<Pair<String, InviteStatus>> ?: mutableListOf()
        )
    }
    ...
    private fun Map<String, Any>.toUsersInvitedToEvents(id: String? = null): UserInvitedToEvents {
        return UserInvitedToEvents(
            ...
            invitedToEvents = this["invitedToEvents"] as? MutableList<Pair<String, InviteStatus>> ?:  mutableListOf()
        )
    }
